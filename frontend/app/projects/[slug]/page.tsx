  ...
  return (
    <Layout>
      <div className="max-w-6xl mx-auto px-5">
        ...
        <article className="mb-8">
          <div className="grid md:grid-cols-2 gap-8">
            <div className="relative mb-2 md:mb-4 sm:mx-0 aspect-square">
              <Image alt={project.title} src={project.coverImage ?? ''} fill className="object-cover object-center" priority />
            </div>
            <div>
              ...
              <div className="hidden md:block md:mb-8 text-slate-600">
                Launched on <DateFormatter dateString={project.publishedAt} /> {project?.author?.name ? `by ${project?.author?.name}` : null}.
              </div>
        ...
        <div className="mb-16">{moreProjects.length > 0 && <ContentGrid title="Other Projects" items={moreProjects} collection="projects" />}</div>
      </div>
    </Layout>
  );
}
...
async function getData({ params }: Params) {
  const db = await load();
  const project = await db.find<Project>({ collection: 'projects', slug: params.slug }, ['title', 'publishedAt', 'description', 'slug', 'author', 'content', 'coverImage']).first();

  const content = await markdownToHtml(project.content);

  const moreProjects = await db.find({ collection: 'projects', slug: { $ne: params.slug } }, ['title', 'slug', 'coverImage']).toArray();
